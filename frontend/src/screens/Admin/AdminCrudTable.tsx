...
function FieldInput({ field, value, onChange }: {
  ...
  if (field.type === 'textarea') {
    return <textarea className={`${base} font-mono`} rows={2} value={String(value ?? '')}
                     onChange={(e) => onChange(e.target.value)} />
  }
  if (field.type === 'number') {
    return <input type="number" step="any" className={base} value={value as number ?? ''}
                  onChange={(e) => onChange(e.target.value === '' ? null : Number(e.target.value))} />
  }
  if (field.type === 'date') {
    return <input type="date" className={base} value={String(value ?? '')}
  ...
  return (
    <>
      <input className={base} value={String(value ?? '')} list={listId}
             onChange={async (e) => {
  ...
  return (
    <div>
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-lg font-bold text-body">{config.title} <span className="text-sm text-muted">({rows.length})</span></h2>
        <button onClick={openNew} className="rounded bg-primary px-3 py-1.5 text-sm font-semibold text-white">+ New</button>
      </div>
      <Card className="p-0"><div className="overflow-x-auto"><table className="w-full text-sm">
        ...
        <tbody>
          {rows.length ? rows.map((r, i) => (
            <tr key={String(r.id)} className={`border-b border-line ${i % 2 ? 'bg-surface-alt' : 'bg-white'}`}>
              ...
              <td className="whitespace-nowrap px-3 py-2 text-right">
                <button onClick={() => openEdit(r)} className="mr-2 text-primary hover:underline">Edit</button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setEditing(null)}>
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
                ...
                <div key={f.name}>
                  ...
                  {f.validateFormula && (
                    <div className="mt-1 flex items-center gap-2">
                      <button onClick={checkFormula} className="rounded border border-line px-2 py-0.5 text-xs">Validate</button>
                      <span className={`text-xs ${formulaCheck.startsWith('✓') ? 'text-green-600' : 'text-red-600'}`}>{formulaCheck}</span>
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => setEditing(null)} className="rounded border border-line px-3 py-1.5 text-sm">Cancel</button>
              <button onClick={save} className="rounded bg-primary px-3 py-1.5 text-sm font-semibold text-white">Save</button>
            </div>
          </div>
        </div>
      )}
