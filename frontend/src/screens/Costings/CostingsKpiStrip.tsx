...
//
// The strip used to fetch ONCE on mount (empty deps), so the Accepted totals + approval rate went
// stale after a costing was accepted/approved/signed-off until a full reload. It now refetches
// whenever a KPI-affecting input moves: it derives a compact signature from the shared costings
// context (count + per-row status:selling_zar — exactly what compute_kpis aggregates) and keys the
// fetch effect on it. CostingsContext replaces the array on each refetch(), so the sig changes on a
// real mutation but NOT on unrelated search/filter/selection churn. (Native landing of backport
// PR #68 / v1.39.4.)
